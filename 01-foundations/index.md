---
title: 01 — Foundations
summary: Why this guide exists, the industry numbers, the contradictory research, and my tracked experience.
---

# Foundations

Start here if you're new to the guide. This folder establishes *why* AI coding is messy to talk about: adoption is near-universal but trust is low, the productivity research contradicts itself, and the practitioner reality is more nuanced than either marketing or skepticism captures.

## Read in order

1. [Why I wrote this](./why-this-guide.md), my stance and warning about opinionated takes
2. [The state of the industry](./state-of-industry.md), adoption numbers, spend, and the trust gap
3. [The research landscape](./research-landscape.md), METR, GitHub/MS, Uplevel, GitClear, Faros, McKinsey
4. [My personal experience](./my-experience.md), tracked observations across languages and tasks

## Where to go next

- Pick a tool → **[02 — Tools](../02-tools/)**
- Learn how to use them well → **[03 — Effective use](../03-effective-use/)**
- Understand the underlying skill shift → **[04 — Understanding & context](../04-understanding-and-context/)**
