...
The opinionated 6-level model in this guide (Levels 0–5: Unmanaged → Sanctioned → Operational → Platformed → Compounding → Native) for org-level AI coding adoption. Synthesizes across Gartner / Deloitte / BCG / A16z while explicitly addressing engineering practice, security integration, junior pipeline, and multi-tool reality. See [Maturity model](./08-team-and-adoption/maturity-model.md) and the [interactive assessment](./08-team-and-adoption/assessment.md).

### AI Gateway
A centralized proxy through which all LLM requests route, providing auth, cost tracking, retention policy enforcement, and audit trail in one place. The Cloudflare-published pattern is the cleanest reference. The single most important platform-team-owned component past Level 2. See [Org design](./08-team-and-adoption/org-design.md).

### AI champions network
The Citi-pioneered model: 5–10% of an engineering org as designated champions with explicit time allocation (30–60 min/week), 1 lead per 10–20 champions. Bridges between the platform team and individual product teams. See [Org design](./08-team-and-adoption/org-design.md#the-ai-champions-network).

### Acceptable Use Policy (AUP), for AI coding
A short policy (one page works) covering approved tools, prohibited data classes, required workflow, audit obligations, and incident response, operationalizing a [STRIDE-style threat model for AI in the SDLC](./08-team-and-adoption/risk-governance-policy.md#the-threat-model--ai-in-the-sdlc-stride-style). See [AUP template](./08-team-and-adoption/templates/aup.md).
